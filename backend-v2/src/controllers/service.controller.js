...
export const createService = async (req, res, next) => {
  try {
    const { category, label, basePrice, minPrice, maxPrice, homeCategory } = req.body
    if (!category || basePrice === undefined) {
      return res.status(400).json({ error: 'category ve basePrice zorunlu' })
    }
    const service = await serviceService.createService({ category, label, basePrice, minPrice, maxPrice, homeCategory })
