...
function MyJobsPage() {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      ...
      in_progress: { text: 'Devam Ediyor', color: 'bg-yellow-100 text-yellow-600' },
      completed: { text: 'Tamamlandı', color: 'bg-green-100 text-green-600' },
      rated: { text: 'Değerlendirildi', color: 'bg-purple-100 text-purple-600' }
    }
        ...
        {displayJobs.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 font-semibold">
              {activeTab === 'active' ? 'Henüz aktif iş yok' : 'Henüz tamamlanan iş yok'}
            </p>
          </div>
        ) : (
